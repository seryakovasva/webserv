...
public class ChangeRasp {
    ...
    private final String UPLOADED_FILE_PATH =
            "/home/sseryakova/webAppProjs/webservice/uploadFile/";

    //Выборка полей для cелектов для определенного преподавателя
...
//            log.info(s);
    ...
    @Produces(MediaType.APPLICATION_JSON) //тип, возвращаемый ресурсом
    @Consumes(value={"application/json"})
    public Response cancelPair (String s, @HeaderParam("authorization") String authorization) {
        ForAuth forAuth = new ForAuth();
        Gson gson =new Gson();
...
//                //есть ли в это время пара у препода
    ...
    @Produces(MediaType.APPLICATION_JSON) //тип, возвращаемый ресурсом
    @Consumes(value={"application/json"})
    public Response recovery (String s, @HeaderParam("authorization") String authorization) {
        ForAuth forAuth = new ForAuth();
    ...
    @Produces(MediaType.APPLICATION_JSON) //тип, возвращаемый ресурсом
    @Consumes(value={"application/json"})
    public Response getPair (String s, @HeaderParam("authorization") String authorization) {
        ForAuth forAuth = new ForAuth();
        Gson gson = new Gson();
        ObjSearch search = gson.fromJson(s, ObjSearch.class);
        TimeWork time = new TimeWork();
...
//            System.out.println(transfer.getId_el() + "id");
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd.MM.yyyy");
            Boolean stop = true;
    ...
    public Response inputFile (MultipartFormDataInput input, @HeaderParam("authorization") String authorization) {
        ...
        if ((forAuth.parseToken(authorization, em).getRole().equals("admin"))
            ...
            //log.info(formParts.get("spring") + "get");
